...
class YearConverters {
  ...
  loadJYears() {
    ...
    this.nowEraCode = "";
    const startYears = this.jYearTuples.map((jYearTuple) => {
      return jYearTuple[3];
    }); // get all starting years
    ...
    for (const jYearTuple of this.jYearTuples) {
      ...
      if (oneEraObject.getEndYear() === 0) {
        this.nowEraCode = oneEraCode;
      } // set the current era
      tempEraCodeAllList.push(oneEraCode);
      if (oneEraObject.getStartYear() >= this.modernEraStart) {
        // include startyears for sorting
    ...
    tempEraCodeModernList.sort(function (a, b) {
      return b[1] - a[1];
    });
    this.eraCodeModernListSorted = tempEraCodeModernList.map((oneTuple) => {
      return oneTuple[0];
    });
  ...
  getNowEra() {
    return this.nowEraCode;
  }
  isNowEra(eraCode) {
    return this.nowEraCode === eraCode;
  }
  getMinYear() {
    return this.minYear;
  }
  ...
  getEName(eraCode) {
    return this.yDict[eraCode].getEName();
  }
  getENameRaw(eraCode) {
    return this.yDict[eraCode].getENameRaw();
  }
  getJName(eraCode) {
    return this.yDict[eraCode].getJName();
  }
  getStartYear(eraCode) {
    return this.yDict[eraCode].getStartYear();
  }
  getEndYear(eraCode) {
    return this.yDict[eraCode].getEndYear();
  }
  getNumYears(eraCode) {
    return this.yDict[eraCode].getNumYears();
  }
  isValidEraCode(eraCode) {
    return this.eraCodeAllListSorted.includes(eraCode);
  }
  isValidIYear(iYear) {
    if (isNaN(iYear)) {
      return false;
  ...
  getHint(eraCode) {
    ...
    const numYears = this.isNowEra(eraCode)
      ? this.getNowYear() - this.getStartYear(eraCode) + 1
      : this.getNumYears(eraCode);
    return "(1-" + numYears + ")";
  }
  /**
  ...
  getEraNamesPlusCodes(eraType) {
    ...
    const listToUse =
      eraType === "modern"
        ? this.eraCodeModernListSorted
        : this.eraCodeAllListSorted;
    for (const eraCode of listToUse) {
      let oneTuple = [
        this.getEName(eraCode) + " " + this.getJName(eraCode),
        eraCode,
      ];
  ...
  iYearToJYear(iYear) {
    ...
    if (iYear < this.minYear || iYear > this.maxYear + 1) {
      return jYears2;
    ...
    jYears1.sort(function (a, b) {
      return a[0] - b[0];
    }); // sort in order by start year
    for (const [startYear, eraCode] of jYears1) {
      jYears2.push([
        this.yDict[eraCode].getEName(),
        this.yDict[eraCode].getJName(),
  ...
  iYearToJYearRefactor(iYear) {
    const jYears = this.iYearToJYear(iYear); // get array of 0, 1 or 2 tuples

    // Unless you need to really do something weird/hacky it is best to avoid using let to declare an array then mutating it in a function. In this case I think it is ok if you just return the array that you need instead of declaring eq :)
    ...
    // Explicitly stating case instead of using else to avoid edge cases
    if (jYears.length === 2) {
      const [eName1, jName1, jYear1] = jYears[0];
      const [eName2, jName2, jYear2] = jYears[1];

      // Note: I like using template literals  for sentences.
      return [
  ...
  jYearToIYear(eraCode, jYear) {
    if (!this.isValidEraCode(eraCode)) {
      return -1;
    } // eraCode not found
    const maxJYear = this.isNowEra(eraCode)
      ? 99
      : this.yDict[eraCode].getNumYears(); // 99 if now era
  ...
  jYearToIYearEq(eraCode, jYear) {
    ...
    if (isNaN(jYear) || parseInt(jYear) === 0) {
      // nothing entered yet
      eq[0] = dispEName + " era";
  ...
  loadZYears() {
    ...
    const zodTuples = [
      // yearmod (mod 12), eName, jName (animal), jName (kanji for zodiac sign)
      [0, "monkey", "猿", "申"],
      [1, "rooster", "鳥", "酉"],
      [2, "dog", "犬", "戌"],
      [3, "boar", "猪", "亥"],
    ...
    for (const zodTuple of zodTuples) {
      // use yearMod as index, overwrite if it already exists
      this.zFromYearDict[zodTuple[0]] = new ZodiacYear(...zodTuple);
    }
  }

  getZodEName(theyear) {
    return this.zFromYearDict[theyear % 12].getEName();
  }
  getZodJName(theyear) {
    return this.zFromYearDict[theyear % 12].getJName();
  }
  getZodJZName(theyear) {
    return this.zFromYearDict[theyear % 12].getJZName();
  }
 ...
 */
export class OneEra {
  // TODO: don't export, use locally only
  ...
  isIYearInEra(iYear) {
    if (this.endYear === 0) {
      return iYear >= this.startYear;
    } else {
      return iYear >= this.startYear && iYear <= this.endYear;
    }
  }
  iYearToEraYear(iYear) {
    if (this.isIYearInEra(iYear)) {
      return iYear - this.startYear + 1;
    } else {
      return 0;
    }
  }
  ...
  getEName() {
    return decodeHtmlCharCodes(this.eName);
  }
  getENameRaw() {
    return this.eName;
  }
